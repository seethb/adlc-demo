...
export default function Pipeline({ s, go }) {
  ...
  return (
    <>
      ...
      {waiting.map(r => (
        <div key={r.id} className="card" style={{ borderColor: 'rgba(245,158,11,0.55)', background: 'linear-gradient(135deg, rgba(245,158,11,0.14), rgba(20,24,42,0.6))' }}>
          <div className="row wrap">
            <PauseCircle size={22} color="#fcd34d" />
            ...
            <div className="feed" style={{ maxHeight: 360 }}>
              {[...logRun.log].reverse().map((l, i) => (
                <div className="feed-item" key={i}>
                  {l.level === 'error' ? <XCircle size={14} color="#fda4af" /> : l.level === 'success' ? <CheckCircle2 size={14} color="#86efac" /> : l.level === 'warn' ? <AlertTriangle size={14} color="#fcd34d" /> : <Zap size={14} color="#a78bfa" />}
...
function StageDrawer({ s, open, onClose, setOpen }) {
  ...
  return (
    <Drawer open onClose={onClose} icon={<Avatar agent={agent} size="lg" busy={st.status === 'running'} />}
      ...
      {st.error && <div className="card" style={{ borderColor: 'rgba(244,63,94,0.5)' }}><b style={{ color: '#fda4af' }}>Error</b> <span className="mono">{st.error}</span></div>}
      {st.status === 'pending' && <Empty>This stage hasn't run yet.</Empty>}

      {st.tokens && (
        <Card title="Meko token economics" icon={<Brain size={16} color="#a78bfa" />} right={st.tokens.reused ? <Badge tone="violet"><Recycle size={11} />artifact reused</Badge> : <Badge tone="green">−{Math.round((1 - st.tokens.meko / Math.max(1, st.tokens.baseline)) * 100)}% input tokens</Badge>}>
      ...
      {st.gate && (
        <Card title="Gate result" icon={<ShieldCheck size={16} color={st.gate.pass ? '#86efac' : '#fda4af'} />} right={<Badge tone={st.gate.pass ? 'green' : 'red'}>{st.gate.pass ? 'PASSED' : 'BLOCKED'}</Badge>}>
          <table className="t">
            <tbody>
              {st.gate.guardrails.map(g => (
                <tr key={g.id}><td style={{ width: 26 }}>{g.pass ? <CheckCircle2 size={15} color="#86efac" /> : g.severity === 'warn' ? <AlertTriangle size={15} color="#fcd34d" /> : <XCircle size={15} color="#fda4af" />}</td><td className="mono" style={{ width: 200 }}>{g.id}</td><td>{g.detail}</td></tr>
              ))}
              {st.gate.evals.map(e => (
                <tr key={e.id}><td>{e.pass ? <CheckCircle2 size={15} color="#86efac" /> : <XCircle size={15} color="#fda4af" />}</td><td className="mono"><FlaskConical size={12} /> {e.id}</td><td><b>{e.value}</b> <span className="dim">≥ {e.threshold}</span> · {e.detail}</td></tr>
              ))}
            </tbody>
          </table>
        </Card>
      ...
      {st.acceptance && (
        <Card title="Acceptance suite" hint={`${st.acceptance.passed}/${st.acceptance.total} · ${st.behavioural?.detail}`} icon={<FlaskConical size={16} color="#a3e635" />}>
          <table className="t"><tbody>{st.acceptance.tests.map((t, i) => <tr key={i}><td style={{ width: 26 }}>{t.ok ? <CheckCircle2 size={15} color="#86efac" /> : <XCircle size={15} color="#fda4af" />}</td><td className="mono" style={{ width: 90 }}>{t.ac}</td><td>{t.name.replace(/^AC-F\d\d-\d+\s*/, '')}</td></tr>)}</tbody></table>
      ...
      {st.approval && <Card title="Release approval" icon={<Rocket size={16} color="#f0abfc" />}><div>{st.approval.rejected ? 'Rejected' : 'Approved'} by <b>{st.approval.by}</b> at {st.approval.at?.slice(0, 19).replace('T', ' ')} UTC{st.approval.note ? ` — “${st.approval.note}”` : ''}</div></Card>}

      {st.recalled?.length > 0 && (
        <Card title={`Recalled from Meko (${st.recalled.length})`} hint="team memory this agent reused instead of re-reading the repo" icon={<Brain size={16} color="#a78bfa" />}>
          <div className="col">
            {st.recalled.map(m => {
      ...
      {st.kb?.length > 0 && (
        <Card title="Knowledge chunks from Meko" icon={<BookOpen size={16} color="#67e8f9" />}>
      ...
      {st.decisions?.length > 0 && (
        <Card title="Decisions written to Meko for the team" icon={<Lightbulb size={16} color="#fcd34d" />}>
      ...
      <Card title="On GitHub" icon={<GitCommit size={16} />}>
        <div className="col">
          ...
          {commits.map(c => <a key={c.sha} className="row" href={`${s.github.url}/commit/${c.sha}`} target="_blank" rel="noreferrer"><span className="mono" style={{ color: '#c4b5fd' }}>{c.sha.slice(0, 7)}</span><span className="ellipsis">{c.message}</span></a>)}
...
function ApproveModal({ s, run, onClose }) {
  ...
  return (
    <Modal open onClose={onClose}>
      <div className="row" style={{ marginBottom: 14 }}><Rocket size={22} color="#f0abfc" /><div><div style={{ fontWeight: 800, fontSize: 17 }}>Release {run.feature} to edge-staging?</div><div className="dim" style={{ fontSize: 12.5 }}>Helm will squash-merge PR #{run.pr} and record a GitHub deployment.</div></div></div>
