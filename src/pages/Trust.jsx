  ...
  return (
    <>
      <div className="grid g4">
        <Stat label="Outbound payloads scanned" icon={<Eye size={14} color="#c4b5fd" />} value={fmt.n(p.scanned)} foot="every prompt, MCP call and GitHub body" />
        <Stat label="PII redactions" icon={<Lock size={14} color="#f0abfc" />} value={fmt.n(p.redactions)} foot={Object.entries(p.byType).map(([k, v]) => `${k} ${v}`).join(' · ') || 'none needed so far'} />
        <Stat label="By destination" icon={<Network size={14} color="#67e8f9" />} value={<span style={{ fontSize: 16 }}><Cpu size={13} /> {p.byDest.claude ?? 0} · <Brain size={13} /> {p.byDest.meko ?? 0} · <Github size={13} /> {p.byDest.github ?? 0}</span>} foot="Claude · Meko · GitHub" />
        <Stat label="Guardrail checks run" icon={<ShieldCheck size={14} color="#86efac" />} value={fmt.n(Object.values(tally).reduce((n, t) => n + t.pass + t.fail, 0))} foot={`${Object.values(tally).reduce((n, t) => n + t.fail, 0)} blocked or warned`} />
      </div>

      <div className="grid g2">
        <Card title="Try the privacy shield" hint="runs locally — nothing here is sent to Claude, Meko or GitHub" icon={<Sparkles size={16} color="#f0abfc" />} glow>
      ...
      <Card title="Guardrails" hint="deterministic checks — block stops the pipeline; warn is reported on the PR" icon={<ShieldCheck size={16} color="#86efac" />}>
        <table className="t">
          ...
          <tbody>
            {Object.entries(s.gates.guardrails).map(([id, g]) => (
              <tr key={id}>
                <td className="mono" style={{ whiteSpace: 'nowrap' }}>{['pii-scan', 'secret-scan'].includes(id) ? <KeyRound size={12} color="#f0abfc" /> : null} {id}</td>
                ...
                <td className="num mono">{tally[id] ? <><span style={{ color: '#86efac' }}>{tally[id].pass}</span> / <span style={{ color: tally[id].fail ? '#fda4af' : 'inherit' }}>{tally[id].fail}</span></> : <span className="dim">—</span>}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div className="grid g-1-2">
        ...
        <Card title="Data classification & transport security" hint="specs/02-design/security.md — also in Meko for every agent" icon={<Network size={16} color="#67e8f9" />}>
