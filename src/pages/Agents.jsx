          ...
          return (
            <div key={a.id} className="card" style={{ cursor: 'pointer', padding: 16 }} onClick={() => setSel(a)}>
              ...
              <div className="grid g2" style={{ gap: 8, fontSize: 12 }}>
                <div><div className="dim">Tokens saved</div><div className="mono" style={{ color: '#7c3aed', fontWeight: 700 }}>{e ? fmt.k(e.baselineIn + e.baselineOut - e.mekoIn - e.output) : '—'}</div></div>
...
function ReuseGraph({ s, edges, onPick }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', height: 400 }}>
      <defs>
        <marker id="arr" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse"><path d="M0,0 L10,5 L0,10 z" fill="#7c3aed" /></marker>
      ...
      <text x={cx} y={cy + 52} textAnchor="middle" fill="#64748b" fontSize="11">Meko datapack iot-edge-adlc</text>
      {edges.filter(e => pos[e.from] && pos[e.to] && e.from !== e.to).map((e, i) => {
        ...
        return <path key={i} d={`M${x1},${y1} Q${mx},${my} ${x2 - (dx / L) * 22},${y2 - (dy / L) * 22}`} fill="none" stroke="#7c3aed" strokeOpacity={0.25 + 0.6 * (e.n / max)} strokeWidth={1 + 4 * (e.n / max)} markerEnd="url(#arr)" className="flow-line"><title>{`${e.from} → ${e.to}: ${e.n} memories`}</title></path>;
      })}
      {nodes.map(n => {
        const [x, y] = pos[n.id];
        return (
          <g key={n.id} transform={`translate(${x},${y})`} style={{ cursor: 'pointer' }} onClick={() => onPick(n)}>
            <circle r={n.id === 'org' ? 24 : 19} fill={n.color} opacity="0.95" stroke="#ffffff" strokeWidth="3" />
            <text y={n.id === 'org' ? 4 : 4} textAnchor="middle" fill="white" fontSize="11" fontWeight="800">{n.name.slice(0, 3)}</text>
            {n.id !== 'org' && <text y={34} textAnchor="middle" fill="#334155" fontSize="11" fontWeight="600">{n.name}</text>}
          </g>
        );
      })}
    </svg>
...
function AgentDrawer({ s, a, onClose, econ }) {
  ...
  return (
    <Drawer open onClose={onClose} icon={<Avatar agent={a} size="lg" />} title={`${a.name} — ${a.role}`} subtitle={`Human owner ${a.owner} · ${a.ownerRole} · Meko agent_id adlc:${a.id}`}>
      ...
      <Card title="Guardrails" icon={<ShieldCheck size={16} color="#16a34a" />}>
      ...
      <Card title={`Memories written by ${a.name} (${mems.length})`} icon={<PenLine size={16} color="#d97706" />}>
